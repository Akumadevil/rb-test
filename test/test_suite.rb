require_relative 'models/processor/abstract_processor_test'
require_relative 'models/processor/processor_factory_test'
require_relative 'models/processor/xml_processor_test'
require_relative 'models/renderer/abstract_renderer_test'
require_relative 'models/renderer/html_renderer_test'
require_relative 'models/renderer/renderer_factory_test'

class TestSuite
end